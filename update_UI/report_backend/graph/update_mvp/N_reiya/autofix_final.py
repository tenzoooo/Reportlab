...
from graph.state import AgentState, MarkdownDocument, QualityIssue, ValidationIssue, now_iso
...
def autofix_final(state: AgentState) -> AgentState:
    stage = "N.autofix_final"
    base_doc = state.markdown.document_styled
    if base_doc is None:
        return state
    ...
    if changed:
        ...
        if state.rendered_docx.retry_count >= 2:
            new_issues.append(
                QualityIssue(
                    code="WARN_DOCX_CONSISTENCY",
                    stage=stage,
                    severity="WARN",
                    message="Retry limit reached; styled markdown updated but re-render is suppressed.",
                    suggested_action="stop",
                )
            )
        else:
            state.rendered_docx.retry_count += 1
            new_issues.append(
                QualityIssue(
                    code="INFO_RETRY_RENDER_DOCX",
                    stage=stage,
                    severity="INFO",
                    message="Styled markdown updated; re-render DOCX is required.",
